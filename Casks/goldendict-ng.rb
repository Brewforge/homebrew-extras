cask "goldendict-ng" do
  ...
  url "https://github.com/xiaoyifang/goldendict-ng/releases/download/v#{version.csv.first}-#{version.csv.second}/GoldenDict-#{version.csv.first}-Qt#{version.csv.third}-#{arch}.dmg",
  ...
  livecheck do
    ...
    regex(%r{/v(\d{2}(\.\d{2}){2})-(\w+\.\h+)/.+Qt(\d+(\.\d+)+)-#{arch}\.dmg$}i)
